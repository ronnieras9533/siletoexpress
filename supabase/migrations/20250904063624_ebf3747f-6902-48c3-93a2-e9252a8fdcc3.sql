-- Fix the trigger function to use proper Supabase service role key
...
BEGIN
  ...
  IF NEW.status IN ('confirmed', 'delivered') AND 
    ...
    VALUES (FORMAT('Order notification triggered for order %s with status %s - HTTP call made', NEW.id, NEW.status));
